...
#define GFX_SPOT_ONE	1//(NUMPP/32)	// Flag indicating spot one on page is used

#define GFX_PREV_SPOT	1//(NUMPP/32)	// Amount to decrement mask to move to last spot
#define GFX_NEXT_SPOT	1//(NUMPP/32)	// Amount to increment mask to move to next spot
...
PRIVATE void _GFXPageAllocVector()
{
	...
	newVec = (gfxPage **)GFX_MALLOC(++g_gfxList.nMaxPages * sizeof(gfxPage*));
...
{
	gfxPage *Page = (gfxPage *)GFX_MALLOC(sizeof(gfxPage));	// Client page to allocate
...
PROTECTED void GFXPageClear()
{
	...
	if(g_gfxList.nMaxPages != 0)	// Check whether client pages exist
		GFX_FREE(g_gfxList.Pages);	// Release page vector
