...
PUBLIC RETCODE TMapDisplay(hTMAP map, int cBeginCol, int cBeginRow, int cEndCol, int cEndRow)
{
	...
	g_p3DDevice->SetFVF(GFXVERTEXFLAG); //nothing cool for now...
	...
	for(int row = cBeginRow; row <= cEndRow; row++)
	{
		for(int col = cBeginCol; col <= cEndCol; col++)
		{
			...
			g_p3DDevice->SetStreamSource(0, cel->vtx, 0, sizeof(gfxVtx));
...
PUBLIC void TMapGetCelVtx(hTMAP map, int ind, float vtx[NUMPTQUAD][eMaxPt])
{
	...
	if(SUCCEEDED(map->cels[ind].vtx->Lock(0,0,(void**)&pVtx, 0)))
	{
...
PUBLIC RETCODE TMapSetTxtSprRand(hTMAP map, hSPRITE spr)
{
	...
	for(int row = 0; row < map->indSize.cy; row++)
	{
		for(int col = 0; col < map->indSize.cx; col++)
		{
			...
			if(SUCCEEDED(thisCel->vtx->Lock(0,0, (void**)&pVtx, D3DLOCK_DISCARD)))
			{
				...
				*/
				...
				_GFXVtxSetTxtCoordAll(&pVtx[0], 
					((float)spr->frameRects[frameInd].left)/txtSize.cx, 
					((float)spr->frameRects[frameInd].top)/txtSize.cy);

				_GFXVtxSetTxtCoordAll(&pVtx[1], 
					((float)spr->frameRects[frameInd].left)/txtSize.cx, 
					((float)spr->frameRects[frameInd].bottom)/txtSize.cy);

				_GFXVtxSetTxtCoordAll(&pVtx[2], 
					((float)spr->frameRects[frameInd].right)/txtSize.cx, 
					((float)spr->frameRects[frameInd].bottom)/txtSize.cy);

				_GFXVtxSetTxtCoordAll(&pVtx[3], 
					((float)spr->frameRects[frameInd].right)/txtSize.cx, 
					((float)spr->frameRects[frameInd].top)/txtSize.cy);
