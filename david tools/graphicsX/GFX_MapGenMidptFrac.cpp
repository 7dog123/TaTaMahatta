							...
							hTXT texture, GFXMATERIAL *mat)
{
	...
	hTMAP newMap = (hTMAP)GFX_MALLOC(sizeof(gfxTMap));
	...
	memcpy(&newMap->material, mat, sizeof(GFXMATERIAL));
	
	//////////////////////////
	//generate the cels of map
	newMap->cels = (gfxCel*)GFX_MALLOC(sizeof(gfxCel)*newMap->indSize.cx*newMap->indSize.cy);
	if(!newMap->cels)
	...
	for(int row = 0; row < newMap->indSize.cy; row++)
	{
		for(int col = 0; col < newMap->indSize.cx; col++)
		{
			...
			*/
			...
			_GFXVtxSetTxtCoordAll(&quadVtx[0], 
				quadVtx[0].x/TextureGetWidth(newMap->texture), 
				quadVtx[0].z/TextureGetHeight(newMap->texture));
			
			quadVtx[1].x = xDir*newMap->celSize[eX]*col;
			quadVtx[1].y = points[dot+size+1];
			quadVtx[1].z = zDir*newMap->celSize[eZ]*(row+1);

			_GFXVtxSetTxtCoordAll(&quadVtx[1], 
				quadVtx[1].x/TextureGetWidth(newMap->texture), 
				quadVtx[1].z/TextureGetHeight(newMap->texture));
			
			quadVtx[2].x = xDir*newMap->celSize[eX]*(col+1);
			quadVtx[2].y = points[dot+size+2];
			quadVtx[2].z = zDir*newMap->celSize[eZ]*(row+1);

			_GFXVtxSetTxtCoordAll(&quadVtx[2], 
				quadVtx[2].x/TextureGetWidth(newMap->texture), 
				quadVtx[2].z/TextureGetHeight(newMap->texture));

			quadVtx[3].x = xDir*newMap->celSize[eX]*(col+1);
			quadVtx[3].y = points[dot+1];
			quadVtx[3].z = zDir*newMap->celSize[eZ]*row;

			_GFXVtxSetTxtCoordAll(&quadVtx[3], 
				quadVtx[3].x/TextureGetWidth(newMap->texture), 
				quadVtx[3].z/TextureGetHeight(newMap->texture));
